...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="testNgXslt.environment"/>
    <xsl:param name="testNgXslt.executiontype"/>
    <xsl:param name="testNgXslt.tagid"/>
    ...
    <xsl:template name="htmlHead">
        <head>
            <title>
                <xsl:value-of select="testng:getVariableSafe($testNgXslt.reportTitle, 'TestNG Reports')"/>
            </title>
	...
	<xsl:function name="testng:testSteps">
        <xsl:param name="reporter-output"/>
        <xsl:variable name="outputString">
				<xsl:for-each select="($reporter-output)">
                        <div>
                            <code>
								<xsl:if test="contains(text(),'[Steps]')">
									<xsl:value-of select="."/>
								</xsl:if>
                            </code>
                        </div>
	...
	<xsl:function name="testng:testExpected">
        <xsl:param name="reporter-output"/>
        <xsl:variable name="outputString">
				<xsl:for-each select="($reporter-output)">
                        <div>
                            <code>
								<xsl:if test="contains(text(),'[Expected]')">
									<xsl:value-of select="."/>
								</xsl:if>
                            </code>
                        </div>
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('overview-chart.svg')}" format="xml">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid meet" width="{$chartWidth}" height="{$chartHeight}"
                 viewBox="0 0 900 300">
                <defs>
                    <style type="text/css">
                        <![CDATA[
                ...
                <svg id="graphzone" preserveAspectRatio="xMidYMid meet" x="0" y="0">
                    ...
                    <g style="stroke:black;stroke-width:1" transform="translate(450,150)">
						...
						<a xlink:href="{testng:suiteContentFileName(.)}" target="content" onclick="javscript:clearAllSelections();">
							<g style="fill:red"
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <body>
                    <h1>
                    Complete Execution Report...<br/></h1>
                    
                    <h3>
                    Environment  =
                    <xsl:choose>
                    <xsl:when test="contains($testNgXslt.environment, '{environment}')">
                    Live
                    </xsl:when>
                    <xsl:otherwise>
                    <xsl:value-of select="$testNgXslt.environment" /> 
                    </xsl:otherwise>
                    </xsl:choose>
                    <br/>
                    ...
                    Execution Type  =
                    <xsl:choose>
                    <xsl:when test="contains($testNgXslt.executiontype,'{executiontype}')">
                    System
                    </xsl:when>
                    <xsl:otherwise>
                     <xsl:value-of select="$testNgXslt.executiontype" /> 
                    </xsl:otherwise>
                    </xsl:choose>
                    <br/>                  
                    ...
                    <xsl:for-each select="reporter-output/line">
                        <div>
                            <code>
                                <xsl:if test="contains(text(),'Device Used =')">
                                    <xsl:value-of select="."/>
                                </xsl:if>
                            </code>
                        </div>
                    ...
                    <xsl:choose>
                    <xsl:when test="contains($testNgXslt.tagid,'&#36;')">
                    </xsl:when>
                    <xsl:otherwise>
                    TagID  =
                    <xsl:value-of select="$testNgXslt.tagid" />
                    </xsl:otherwise>
                    </xsl:choose>
                    <br/>
                        ...
                        <table width="100%" cellpadding="5" cellspacing="1">
                            <tr style="background-color: #eaf0f7;">
                                <td width="100%">
                                    ...
                                    <xsl:value-of select="@name"/> Execution Results..
                                </td>
                ...
                <body>
                  ...
                  <!--   <div>
                        <a href="overview.html" target="content"
                           onclick="javscript:clearAllSelections();"><b><h3>Results Homepage</h3></b>
                        </a>
                    </div>--> 
        ...
        <div id="testMethodsByStatus">
            ...
            <table class="testMethodsTable" cellpadding="0" cellspacing="0">
                <tr class="methodsTableHeader">
                    ...
                    <td>Exception</td>
					<td nowrap="true">TestCase_Description</td>
					<td nowrap="true">Status(Screenshot,if FAIL)</td>
                </tr>
            ...
            <tr id="{concat($methodId, '_row')}" class="{testng:testMethodStatus(.)}">
                ...
                <td nowrap="true" width="50px">
                    <xsl:if test="./exception">
                        <a onclick="toggleDetailsVisibility('{$exceptionDetailsId}')">
                   			 ...
                   			 <xsl:if test="contains(exception/@class, 'Assertion')">
                   			 	<b><xsl:value-of select="exception/message"/></b>
                   			 </xsl:if>
                        </a>
                    </xsl:if>
                </td>
				<td nowrap="true">
					<xsl:for-each select="reporter-output/line">
                        <div>
                            <code>
							<xsl:if test="contains(text(),'[BriefDesc]')">
                                <xsl:value-of select="."/>
							</xsl:if>
                            </code>
                        </div>
				...
				<td nowrap="true">
					<xsl:for-each select="reporter-output/line">
                        <div>
                            <code>
							<xsl:choose>
							<xsl:when test="contains(text(),'[Status]') or contains(text(),'[End]') or contains(text(),'[Snapshot]') or contains(text(),'\screenshots\')">
							<xsl:choose>
                                <xsl:when test="contains(text(),'\screenshots\')">
                    ...
                    <div id="{$detailsId}" class="testMethodDetails">
                      ...
                      <!--  <xsl:call-template name="formField">
						...
						<xsl:if test="./reporter-output">
                            <xsl:call-template name="formField">
                                <xsl:with-param name="label" select="'TestSteps'"/>
                                <xsl:with-param name="value" select="testng:testSteps(./reporter-output/line)"/>
                            </xsl:call-template>
                        </xsl:if>
						<xsl:if test="./reporter-output">
                            <xsl:call-template name="formField">
                                <xsl:with-param name="label" select="'Expected Result'"/>
                                <xsl:with-param name="value" select="testng:testExpected(./reporter-output/line)"/>
                            </xsl:call-template>
                        </xsl:if>
